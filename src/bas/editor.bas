...
COLOUR128-(I%=M%):COLOUR3:PRINTTAB(4,25+I%);:IFI%?&2B00<>0THENPRINT"*";ELSEPRINT" ";
...
IFC%=0THENPROCu(V%+X%,W%+Y%,18):PROCu(E%+T%,H%,18)ELSEPROCo(X%,Y%):PROCu(E%+T%,H%,T%)
...
IFK%=81THENIFT%>0THENPROCu(E%+T%,H%,T%):C%=1:D%=1:T%=T%-1:ENDPROC
IFK%=87THENIFT%<17THENPROCu(E%+T%,H%,T%):C%=1:D%=1:T%=T%+1:ENDPROC
...
V%=3:W%=1:E%=4:H%=23:T%=1:X%=0:Y%=0:C%=1:D%=1:M%=0:m$=""
