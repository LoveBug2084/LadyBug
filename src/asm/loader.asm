...
;						setup color palette
;						put ula into 16 color 2MHz mode
;-----------------------------------------------------------------------------------------------------------------------------------------------------
...
.swrInitScreenPalette
	...
	lda #%11110100				; put ula into 16 color 2MHz mode
	sta ulaMode

	;---------------------------------------------------------------------------------------------------------------------------------------------
	; wait 0.5 seconds for monitor to resync
	;---------------------------------------------------------------------------------------------------------------------------------------------

	lda #pause * 0.5			; set pause time to 0.5 seconds
	sta pauseCounter
	
.swrInitScreenWait				; repeat

	lda pauseCounter			; until pause time has expired
	bne swrInitScreenWait
