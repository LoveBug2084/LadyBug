...
IF F%?2 = V% THEN CALL HIMEM ELSE OSCLI("LOAD LBConf")

V%=0:FOR Z%=&00 TO &7C:V%=(V%+(Z%?H% EOR M%)) AND &FF:NEXT Z%
IF V% <> H%?&7D THEN OSCLI("LOAD LBConf")
...
IF H$="Y" OR S$="Y" THEN V%=0:FOR Z%=&00 TO &7C:V%=(V%+(Z%?H% EOR M%)) AND &FF:NEXT Z%:H%?&7D=V%:OSCLI("SAVE LBConf " + STR$~(&FF0000 + H%) + " +7E"):F%?0=0:F%?1=0:F%?2=0
